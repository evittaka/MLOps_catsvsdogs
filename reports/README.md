...
Additionaly, as some work was done on the google cloud the new member should be added to the GC project using their account.
...
The first challenge met was how to collaborate in a group of five, in a structured and productive way. There were many different features that could both be developed independently, or were dependent on each others. Normally in a project, the focus was the model iteself, so it is more straightforward. To support this, a Trello board (list of tasks) was made, such that people could see what each person was working on, which tasks dependen on each others, and if the task was done and waiting as a pull request. This helped keeping the projet more structured.

In regards to the technical aspects, the biggest challenges were probably the CD on Google Cloud, as this expanded the project from VSCode+GitHub to a tertiary location with many things to set up. The cloud run is a very powerfull tool, and when the first edition was deployed, it was easier to make modifications to the process to account for changes to the project. We initially started by cloud deployment of the model, which expanded to the CD. Then followed the deployment of the API and the frontend.

What made this process much more managable was the integration with GitHub Actions to see if your current commits to a branch would succeed or fail. This enabled us to keep a working main before integrating new features, and each group member could work on their own pace to make their feature work in the framework.
